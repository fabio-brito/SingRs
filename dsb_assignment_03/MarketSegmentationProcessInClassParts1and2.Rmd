...
# All parameters of the report are in this code chunk
...
#minimum_variance_explained = 5  # between 1 and 100
...
segmentation_attributes_used = c(10, 19, 5, 12, 3, 24, 26, 11, 30, 2)
...
```{r setupdata2E, echo=TRUE, tidy=TRUE}
# Some other parameters regarding the printout of the report

# Minimum number below which you would like not to print - this makes the readability of the tables easier. Default values are either 10e6 (to print everything) or 0.5. Try both to see the difference.
MIN_VALUE = 0.5

# Maximum number of observations to show in the report and slides. 
...
2. What do these correlations imply about the possibility of performing factor analysis?

**Answers:**

1 - Yes, they make sense. Correlations are listed below:  
* Q.3/9/26:   High personal status (ego)  
* Q.6/7:      Rewarding yourself for hardwork (success)  
* Q.16/17/20: Professional or Enthusiast relation with boats  
* Q.20/27/28: Passionate and knowledgeable about the subject  

2 - It suggests that we don't need to include every single question as a factor in the analysis, because those questions will address the same factor  
...
2. What characteristic (e.g. about the shape) of the plot above would we prefer to see? Why?

**Answers**

1 - The table shows eigenvalues, which indicate what percentage of variation in the dataset is explained by a few key components. Looking for eigenvalues bigger than one or a cumulative percentage of explained variance above 65% is a good threshold.  

2 - The plot visually shows the "elbow" of the data, determining a good starting point to analyze how many factors we should include in our analysis. Also, it provides the "1" baseline, setting a cutting point for variables that provide very little explanation for the variance (diminishing returns). Ideally, as few components as possible should have as high eigenvalues and cumulative variance explanation as possible.  
...
3. What lessons about data science do you learn when doing this analysis? Please comment. 

**Answers**

1 - Yes, they make business sense, because those Factors are dimensions clients evaluate when purchasing a boat. Factor 1 indicates prestige and social status associated with boating. Factor 2 indicates adventure and explorations associated with boating. Finally, Factor 3 portrays brand equity, meaning how the brand conveys value.

2 - Having used eigenvalues, 3 variations in the manual settings, and minimum variance-explained threshold of 20% and 5%, we conclude that eigenvalues provide strongest statistical, business and interpretative  arguments for factor selection criteria. On the basis of eigenvalues we conclude there are 5 different factors explaining 52.74% of the cumulative variance in the dataset. Furthermore, we also included 5 extra questions that scored very low for all factors to insure any variability they uniquely represent does not escape from our segmentation.  

3 - Data science provides us with statistically robust tools to consume information. The interpretation of the results however should be based on business acumen and how actionable (e.g., whether the segmentation  allows to identify and target mutually exclusive customer segments for an optimal allocation of resources) the insights from the data are. It is also important to ensure that elements that loads onto factors present a cohesive picture of a targeted motivation or dimension, meaning that factors should overlap as less as possible. Nevertheless, only information that aids decision making or that changes current decision has an actual value. 
...
1. Which of the 29 initial variables would you select to represent each of the factors you selected?  

**Answers**

Considering the initial strategy of picking only the questions with the biggest factor loading, we derived the following list as segmentation attributes: 
Component 1 - Q1.9  
Component 2 - Q1.18  
Component 3 - Q1.4  
Component 4 - Q1.11  
...
However, to not lose any important differences, we included variables that failed to feature a high loading for any of the factors. Therefore, we included the following questions on the segmentation:  

Q.1, Q.10, Q.23, Q.25, Q.29   
...
2. Based on the tables above, what are some key features of each of the segments of this solution? In other words, for some of the segments, what are some features of these segments that are very different from all/most other segments or from the average of the population?

**Answers**

1 - The questions Q1.1 to Q1.29 represent the relative strength a certain need or motivation has for members of that segment. The questions Q2 to Q18 represent certain demographics and purchase process and behaviours that overrepresent in certain segments. The numbers with the largest absolute values are the most informative. 

2 - Relative to the population, on average:   
OBS: Segment letter is not tied to the segment numbers, since they rotate every time the document is Knit  
Segment A: Q1.11 (-1.15) - not engaged enough to repair own boat  
Segment B: Q1.25 (-1.12) - do not care about being in charge of the boat  
Segment C: Q1.9  (-0.70) - do not associate the boat with social status  
Segment D: Q1.2  (+1.19) - values lower price over brand  
Segment E: Q1.23 (+0.78) - believe their boat allows them to excel in sports   
...
2. Can you describe some of the segments you found based on their profiles?
3. Can you now answer the [Boats case questions](http://inseaddataanalytics.github.io/INSEADAnalytics/Boats-A-prerelease.pdf)? What business decisions do you recommend to this company based on your analysis?

**Answers**

1 - Statistically, the elbow for the plot of distances can be observed around 5 and 6 segments. Beyond that, the distances are not significant to justify the creation of more clusters. Interpretation and business wise, we attempted clustering with 3, 4, 5, and 6 segments and found that 5 segments present the most cohesive picture in terms of deferring attitudes and motivations towards boating.  

2 -
OBS: Segment letter is not tied to the segment numbers, since they rotate every time the document is Knit  
OBS2: Age can potentially be a strong profiling variable. However, it is scored in the survey between 1-4, but we don't know what they represent.

Segment B: They consider themselves the least experienced with boats (Q8) and own or are looking for the boats with lower HP (Q5).  

Segment D: They are as experienced with boats as the population (Q8), own or are looking for the boats with highest HP (Q5), and are significantly younger than the population (Q15)

Segment E: They see themselves as experts in terms of boat experience (Q8), own or are looking for boats with higher HP (Q5), and will probably buy a new boat in the future (Q3).  

3 - We recommend the company to target each segment considering it most important features. 

Advertising media - the segment that values adventure should be target with ads that portray excitement, adventure, handling fast boats, and maybe even thrilling scenarios. The segment that values status should be targeted with fancy scenarios showing other symbols of prestige such as expensive cars, luxury brands, etc. 

Product lines - brand conscious segments can be targeted via prestigious brands differentiated from down-market brands. Vice versa, product lines targeting economy-seekers can be undifferentiated, designed for economy and scale with basic features.

Marketing channels - certain segments visit a lot of dealerships to buy, indicating the company could benefit from having presence at the point of sale and bolstering dealer relationships to push their products.



**You have now completed your first market segmentation project.** You can also use this report template for other data now...