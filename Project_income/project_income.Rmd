...
data_training$more_50k <- as.integer(data_training$more_50k)
...
data_testing$more_50k <- as.integer(data_testing$more_50k)
...
```{r}
linear_model <- lm(more_50k ~ . -education, data=data_training)
...
```{r glm}
model_logistic_glm <- glm(more_50k ~ . -education, data=data_training, family="binomial"(link="logit"))
...
logistic_classification[logistic_probabilities>1-0.76]="1"
...
```{r ctree}
ctree_tree<-ctree(more_50k ~ . - education,data=data_training) 
...
ctree_classification[ctree_probabilities[,2]>1-0.76]="1" 
