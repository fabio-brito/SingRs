...
pacman::p_load("caret","ROCR","lift","glmnet","MASS","e1071", "randomForest", "xgboost", "party", "partykit","ROCR","lift","rpart","e1071") 
```
...
CCDdata_A$LIMIT_BAL <- as.factor(CCDdata_A$LIMIT_BAL)
...
confusionMatrix(ctree_classification,testing$default.payment.next.month,positive = "1")
```
...
rpart_tree<-rpart(default.payment.next.month~.,data=training, method="class", control=CART_cp) #Run ctree on training data
...
confusionMatrix(rpart_prediction_class,testing$default.payment.next.month,positive = "1") #Display confusion matrix
...
plotLift(rpart_prediction_class,  testing$default.payment.next.month, cumulative = TRUE, n.buckets = 10) # Plot Lift chart
```

#Random Forest

```{r random_forest}
model_forest <- randomForest(default.payment.next.month~ ., data=training, 
                             importance=TRUE,proximity=TRUE,
                             type="classification") #cutoffs need to be determined for class 0 and class 1. By default 50/50, but need not be those necessarily
print(model_forest)   
plot(model_forest)
...
confusionMatrix(forest_classification,testing$default.payment.next.month, positive="1") #Display confusion matrix. Note, confusion matrix actually displays a better accuracy with threshold of 50%
...
#### Lift chart
plotLift(forest_probabilities[,2],  testing$default.payment.next.month, cumulative = TRUE, n.buckets = 10) # Plot Lift chart
...
confusionMatrix(as.factor(ifelse(XGboost_prediction>0.6073,1,0)),testing$default.payment.next.month,positive="1") #Display confusion matrix
...
#### Lift chart
plotLift(XGboost_prediction, testing$default.payment.next.month, cumulative = TRUE, n.buckets = 10) # Plot Lift chart
